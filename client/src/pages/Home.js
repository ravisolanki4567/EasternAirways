...
class Home extends Component {
  ...
  async componentDidMount() {
    await api
      ...
      .then(() => {
        ...
        if (searchObject) {
          this.handleSearch(
            searchObject.adultCount,
            searchObject.childCount,
            searchObject.deptAirport,
            searchObject.arrAirport,
            searchObject.deptDate,
            searchObject.retDate,
            searchObject.cabinClass
          );
        }
      });
  ...
  handleSearch(
    ...
    if (
      ...
      for (let i = 0; i < flightArr.length; i++) {
        for (let j = 0; j < flightArr.length; j++) {
          if (
            ...
            Date.parse(flightArr[i].ArrivalDate) <
              Date.parse(flightArr[j].DepartureDate)
          ) {
      ...
      this.setState(
        {
          tripArr: resultArr.filter(
            (p) =>
              p[0].DepartureAirport === deptAirport &&
              p[0].ArrivalAirport === arrAirport &&
              Date.parse(p[0].DepartureDate) === Date.parse(deptDate) &&
              Date.parse(p[1].ArrivalDate) === Date.parse(retDate) &&
              p[0][cabinClass]["AvailableSeats"] >=
                Number(adultCount) + Number(childCount) &&
              p[1][cabinClass]["AvailableSeats"] >=
                Number(adultCount) + Number(childCount)
          ),
        },
            ...
            tripArr && tripArr.length > 0 ? (
              tripArr.map((t) => (
                <Trip
                  deptFlight={t[0]}
                  arrFlight={t[1]}
                  deptCabin={this.deptCabin}
                  arrCabin={this.arrCabin}
              ...
              <NoResults
                deptAirport={this.state.deptAirport}
                arrAirport={this.state.arrAirport}
                deptDate={this.state.deptDate}
                arrDate={this.state.retDate}
              />
            )
            /*  */
          }{" "}
